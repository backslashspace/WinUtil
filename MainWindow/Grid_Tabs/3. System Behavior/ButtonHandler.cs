...
namespace WinUtil.Grid_Tabs
{
    public partial class BehaviorGrid
    {
        ...
        private async void HyperKey_UnReg(object sender, RoutedEventArgs e)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    String exePath = "C:\\Program Files\\WinUtil\\HyperKey_UnReg.exe";
                    ...
                    File.Copy(Resource_Assets.HyperKey_UnReg_PathName, exePath, true);
                    ...
                    if (globalUserInitString.Contains($"\"{exePath}\""))
                    {
                    ...
                    {
                        xRegistry.SetValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon", "Userinit", $"{globalUserInitString}, \"{exePath}\",", RegistryValueKind.String);
                    }

                    LogBox.Add("Deregistering Teams and Office Hyper keys");
                    xProcess.Run(exePath, waitForExit: true);
