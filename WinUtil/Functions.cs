    ...
    public class Functions : MainWindow
    {
        ...
        public static void General()
        {
            try
            {
                ...
                PowerShell.Create().AddScript("Add-MpPreference -ExclusionProcess \"C:\\Program Files\\VeraCrypt\\VeraCrypt.exe\"").Invoke();
                ...
                //Write("Diableing active network probing\n", "gray", null);
                //Registry.SetValue(@"HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\NetworkConnectivityStatusIndicator", "NoActiveProbe", 1, RegistryValueKind.DWord);
                ...
                PowerShell.Create().AddScript("Get-ChildItem -Path \"C:\\Users\\" + UserPath + "\" -Force | Where-Object { $_.LinkType -ne $null -or $_.Attributes -match \"ReparsePoint\" } | remove-item -force -ErrorAction SilentlyContinue").Invoke();
                PowerShell.Create().AddScript("Get-ChildItem -Path \"C:\\Users\\" + UserPath + "\\documents\" -Force | Where-Object { $_.LinkType -ne $null -or $_.Attributes -match \"ReparsePoint\" } | remove-item -force -ErrorAction SilentlyContinue").Invoke();
                ...
                if (!IsWin11Server22Complient)
                {
                    ...
                    if (!isinstalled)
                    {
                        ...
                        if (CompareHash256("assets\\" + Const.VCLibsName, Const.VCLibs, true)[0] == 1 && CompareHash256("assets\\Windows Terminal\\" + Const.WTName, Const.WT, true)[0] == 1 && CompareHash256("assets\\Windows Terminal\\" + Const.WTLicenseName, Const.WTLicenseHash, true)[0] == 1)
                        {
                            ...
                            PowerShell.Create().AddScript("Add-ProvisionedAppPackage -Online -PackagePath \"assets\\Windows Terminal\\" + Const.WTName + "\" -LicensePath \"assets\\Windows Terminal\\" + Const.WTLicenseName + "\"").Invoke();
        ...
        public static void BlockTCP()
        {
            try
            {
                ...
                if (result0 == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    PowerShell.Create().AddScript("New-NetFirewallRule -DisplayName \"Block VMRDP & WSDAPI\" -Direction Inbound -LocalPort 2179,5357 -Protocol TCP -Action Block -Description \"Blocks external access of TCP 5357 and 2179, created by WinUtil.\"").Invoke();
                }
                else if (result0 == System.Windows.Forms.DialogResult.No)
                {
                    Write("Blocking external TCP access to port 2179 (VMRDP)\n", "darkgreen", null);
                    PowerShell.Create().AddScript("New-NetFirewallRule -DisplayName \"Block VMRDP\" -Direction Inbound -LocalPort 2179 -Protocol TCP -Action Block -Description \"Blocks external access of TCP 2179, created by WinUtil.\"").Invoke();
                }
                else
                {
                    Write("Nothing changed, skipping\n\n", "darkyellow", null);
        ...
        public static void RMOneDrive()
        {
            try
            {
                if (File.Exists(@"C:\Users\" + UserPath + @"\AppData\Local\Microsoft\OneDrive\OneDrive.exe") || File.Exists("C:\\Windows\\SysWOW64\\OneDriveSetup.exe") || File.Exists("C:\\Windows\\System32\\OneDriveSetup.exe"))
                {
                    ...
                    Start("takeown.exe", "/f \"C:\\Users\\" + UserPath + "\\OneDrive\"", true, true);
                    Start("cacls.exe", "\"C:\\Users\\" + UserPath + "\\OneDrive\" /E /g " + AdminGroupName + ":f", true, true);
                    Start("powershell.exe", "\"Remove-Item \"C:\\Users\\" + UserPath + "\\OneDrive\" -Recurse -Force\"", true, true);
        ...
        public static void Firefox()
        {
            try
            {
                ...
                Start("temp\\7z.exe", "x \"assets\\Firefox\\" + profileid + ".zip\" -o\"C:\\Users\\" + UserPath + "\\AppData\\Roaming\\Mozilla\" -y", true, true);
            }
            catch (Exception ex)
            {
                Write(ex.Message + "\n\n", "red", null);
                ...
                try
                {
                    if (Directory.Exists(@"C:\Program Files (x86)\Microsoft\EdgeUpdate") || Directory.Exists(@"C:\Users\" + UserPath + @"\AppData\Local\Microsoft\EdgeUpdate"))
                    {
                        ...
                        try
                        {
                            ...
                            Start("cacls.exe", "\"C:\\Users\\" + UserPath + "\\AppData\\Local\\Microsoft\\EdgeUpdate\" /E /g " + AdminGroupName + ":f", true, true);
                            Start("taskkill.exe", "/f /im MicrosoftEdgeUpdate.exe", true, true);
                            Thread.Sleep(500);
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (input == "5")
                    {
                        ...
                        {
                            var result0 = System.Windows.Forms.MessageBox.Show(
                            "Only supported on Windows 11, Windows Server 2022 and newer. Use Option 4 instead.",
                            "Invalide Input",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                ...
                if (option == "5" || option == "4" || option == "3" || option == "2" || option == "1" || option == "0")
                {
                    ...
                    if (IsWin11Server22Complient)
                    {
                        ...
                        PowerShell.Create().AddScript("Set-SmbServerConfiguration -EncryptionCiphers \"AES_256_GCM, AES_128_GCM\" -Confirm:$false").Invoke();
                    }
                    else
                    {
                        Write("Skipping Encryption Ciphers, OS to old\n", "darkgray", null);
                    }

                    Write("Deactivating SMB 1\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -EnableSMB1Protocol $false -Confirm:$false").Invoke();

                    Write("Enable Smb encryption on secure connection\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -DisableSmbEncryptionOnSecureConnection $false -Confirm:$false").Invoke();
                }
                if (option == "5" || option == "4" || option == "3" || option == "2" || option == "1")
                {
                    Write("Enable Smb encryption\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -EncryptData $true -Confirm:$false").Invoke();
                }
                if (option == "5" || option == "4" || option == "3" || option == "2")
                {
                    Write("Rejecting unencrypted Smb access\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -RejectUnencryptedAccess $true -Confirm:$false").Invoke();
                }
                if (option == "5" || option == "4" || option == "3")
                {
                    Write("Deactivating autoshares\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -AutoShareWorkstation $false -Confirm:$false").Invoke();
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -AutoShareServer $false -Confirm:$false").Invoke();
                }
                if (option == "5" || option == "4")
                {
                    Write("Require traffic signature\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -EnableSecuritySignature $true -Confirm:$false").Invoke();
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -RequireSecuritySignature $true -Confirm:$false").Invoke();
                }
                if (option == "5")
                {
                    Write("Set Encryption to AES 256 GCM\n", "gray", null);
                    PowerShell.Create().AddScript("Set-SmbServerConfiguration -EncryptionCiphers \"AES_256_GCM\" -Confirm:$false").Invoke();
                }
            }
            catch (Exception ex)
            {
        ...
        public static void DeactivateEncryptSMB()
        {
            try
            {
                ...
                PowerShell.Create().AddScript("Set-SmbServerConfiguration -EncryptionCiphers \"AES_256_GCM, AES_128_GCM, AES_256_CCM, AES_128_CCM\" -Confirm:$false").Invoke();

                PowerShell.Create().AddScript("Set-SmbServerConfiguration -EncryptData $false -Confirm:$false").Invoke();

                PowerShell.Create().AddScript("Set-SmbServerConfiguration -RejectUnencryptedAccess $false -Confirm:$false").Invoke();

                PowerShell.Create().AddScript("Set-SmbServerConfiguration -RequireSecuritySignature $false -Confirm:$false").Invoke();

                PowerShell.Create().AddScript("Set-SmbServerConfiguration -AutoShareWorkstation $true -Confirm:$false").Invoke();

                PowerShell.Create().AddScript("Set-SmbServerConfiguration -AutoShareServer $true -Confirm:$false").Invoke();
            }
            catch (Exception ex)
            {
                Write(ex.Message + "\n\n", "red", null);
        ...
        {
            Start("explorer.exe", "\"C:\\Users\\" + UserPath + "\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\"", false, false);
        }
