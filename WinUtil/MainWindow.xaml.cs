...
namespace WinUtil
{
    public partial class MainWindow : Window
    {
        ...
        public class Const
        {
            public const string FirefoxImageName = "Firefox Setup 110.0.1.exe";
            public const string FirefoxImageHash = "2279b0fecab9b381bd016570d715b62281828819805987f9c0ecdfaf22cca17c";
        ...
        public class ThreadIsAlive
        {
            ...
            public static bool BlockTCP { get; set; }
        ...
        public static void SetRegLicense()
        {
        ...
        public static string[] ExecuteAPowershell(string RawPSCommand, bool FormatCustom, int RMTop, int RMBottom)
        {
            List<string> Output;
            string PSOut;
            ...
            if (FormatCustom)
            {
                PSOut = (PowerShell.Create().AddScript(RawPSCommand + " | Format-Custom | Out-String").Invoke())[0].ToString();
            }
            else
            {
                PSOut = (PowerShell.Create().AddScript(RawPSCommand + " | Out-String").Invoke())[0].ToString();
            }
            ...
            for (int i = 0; i < RMTop; i++)
            {
                Output.RemoveAt(0);
            }

            for (int i = 0; i < RMBottom; i++)
            {
        ...
        {
            //returns 0 when not found / not usable
            //returns 1 when found / after installing + usable
            ...
            for (int i = 0; i < Folders.Length; i++)
            {
                if (Folders[i].Contains("Microsoft.DesktopAppInstaller"))
                {
                    if (Regex.IsMatch(Folders[i], "([0-9]+)\\.([0-9]+)"))
                    {
                        ...
                        try
                        {
                            if (Int32.Parse(arr[1]) >= 1 && Int32.Parse(arr[2]) >= 20)
                            {
                                IsUpToDate = true;
                            }
...
#pragma warning restore IDE0059
        ...
        public MainWindow()
        {
            ...
            try
            {
                ...
                {
                    ThreadStart childref = new ThreadStart(SetRegLicense);
                    Thread childThread = new Thread(childref);
                    childThread.Start();
                }

                //check systemtype
                if (ExecuteAPowershell("$env:firmware_type", false, 0, 1)[0].ToUpper() == "UEFI")
                {
                    IsUEFI = true;
                    Write("Detected UEFI\n\n", "cyan", null);
                }
                ...
                try
                {
                    if (SafeReadRegString("hklm", @"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "InstallationType", 1).ToLower().Contains("server"))
                    {
                        ...
                        if (int.Parse(SafeReadRegString("hklm", @"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "CurrentBuildNumber", 1)) >= 20348)
                        {
                            IsWin11Server22Complient = true;
                        }
                    }
                    else if (int.Parse(SafeReadRegString("hklm", "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", "CurrentBuildNumber", 1)) >= 22000)
                    {
                        IsWin11Server22Complient = true;
                ...
                catch (Exception)
                {
                    ...
                    Write("CurrentBuildNumber || InstallationType\n\n", "darkgray", null);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SelectWLanDir(object sender, RoutedEventArgs e)
        {
            ...
            ofd.SelectedPath = "C:\\Users\\" + UserPath;
        ...
        private void Debug(object sender, RoutedEventArgs e)
        {
            ...
            //string domain = System.Net.NetworkInformation.IPGlobalProperties.GetIPGlobalProperties().DomainName.Split('.')[0];
            //User = System.Security.Principal.WindowsIdentity.GetCurrent().Name.Split('\\')[1];
